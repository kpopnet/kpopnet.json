...
def get_profiles_path():
    return path.join(get_root_path(), 'profiles')


def get_images_path():
    return path.join(get_root_path(), 'images')


def check_name(name):
    assert (
        name != '.' and name != '..' and '/' not in name and name != INDEX_NAME
    ), 'Bad name'
...
def dump_json(d):
    s = json.dumps(
        d, ensure_ascii=False, sort_keys=True, indent=4, default=default_encoder
    )
    s += '\n'
