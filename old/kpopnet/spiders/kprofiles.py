...
class KprofilesSpider(ProfileSpider):
    ...
    def parse_band(self, response):
        ...
        for p in response.css('.entry-content > p'):
            ...
            # Idol info paragraph.
            elif p.css('span::text').re_first(r'(?i)(stage|real|birth)\s+name'):
    ...
    def parse_idol(self, response, band, p):
        ...
        for span in p.css('span'):
            ...
            if not val or not val.strip():
                val = span.xpath('following::text()/following::text()').extract_first()
                if not val:
                    continue
            key = key.strip()
            val = val.strip()
    ...
    def normalize_idol(self, idol):
        ...
        with suppress(AttributeError):
            name, alt_name = re.match(
                r'(.*?)\s*\(.*?known\s+as\s+(.*?)\)', name
            ).groups()
            idol['name'] = name
        ...
        # Alt name var 3.
        with suppress(AttributeError):
            name, alt_name = re.match(r'(.*?)\s+or\s+(.*)', name).groups()
            idol['name'] = name
            idol['alt_names'] = [alt_name]

        # Alt name var 4.
        with suppress(AttributeError):
            name, alt_name = re.match(r'(.*?)\s*/\s*(.*)', name).groups()
            idol['name'] = name
            idol['alt_names'] = [alt_name]

        # Hangul name.
        with suppress(AttributeError):
            name, name_hangul = re.match(r'(.*?)\s*\((.*?)\)', name).groups()
        ...
        with suppress(KeyError, AttributeError):
            ...
            birth_name, korean_name = re.match(
                r'(.*?)\s*\{(.*?)\}', birth_name
            ).groups()
        ...
        with suppress(KeyError, AttributeError):
            birth_name = idol['birth_name']
            birth_name, alt_name = re.match(
                r'(.*?)\s*\(.*?english\s+name\s+is\s+(.*?)\)', birth_name, re.I
            ).groups()
            idol['birth_name'] = birth_name
            idol['alt_names'] = [alt_name]
        ...
        with suppress(KeyError, AttributeError):
            birth_name = idol['birth_name']
            birth_name, birth_name_hangul = re.match(
                r'(.*?)\s*\((.*?)\)', birth_name
            ).groups()
        ...
        with suppress(KeyError, AttributeError):
            ...
            korean_name, korean_name_hangul = re.match(
                r'(.*?)\s*\((.*?)\)', korean_name
            ).groups()
            idol['korean_name'] = korean_name
